...
#define		VREFIN		((uint16_t*)((uint32_t)0x1FFF7A2A))
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
	if(__HAL_ADC_GET_FLAG(&hadc1,ADC_FLAG_EOC) != RESET)
	{
		...
		if(count == 3)
			count = 0;
		Vdda = (float)3.3 *(*VREFIN) / adc1_value[1];
		Vadc1 = Vdda * adc1_value[0] / 4095;
		Vsense = Vdda * adc1_value[2] / 4095; // Gerçek voltaj değerinden Sensöre değerine ulaştık
